...
**当前版本：V5.1**（人格检索分层 + 数据扩充：偏好第 5 路、核心记忆层、梗库双路由、偏好 22 条 + 核心记忆 5 条）。完整重构历程见 `ROADMAP.md`。
...
- **人格检索分层（V5.1）**：偏好第 5 路语义检索（属性事实，聊到才注入，阈值 0.55）；核心记忆层（印象最深的结晶，低阈值高浮现，注入【她的核心记忆】）；梗库双路由（关键词粗筛 + LLM 语境确认，防误触发）
- **人格数据扩充（V5.1）**：偏好 22 条一条一话题（食物/水果/果冻/饮料/雨天/看书/电话/动物/瓜类/运动/作家/番剧/游戏…）+ 核心记忆 5 条（三好学生 38票、考试 222名、虚拟深圳上学、作家梦《乌色月》含原文）
- **好友自动通过**：收到好友申请自动同意，新加绿冻立即变双向好友，B站推送可达
- **离线工具箱**：`run_tool.py` 统一入口，15 个子命令覆盖从"直播音频"到"人格数据"的完整蒸馏流程（含 `bili-check` / `vision-test` / 偏好 / 核心记忆预计算）
...
- **V5.1（2026-08-10，人格检索分层 + 数据扩充）**：
  - 检索分层：偏好第 5 路（语义检索，属性命中才注入，阈值 0.55）；核心记忆层 core_stories（印象最深的结晶，低阈值高浮现）；梗库双路由（关键词粗筛 + LLM 语境确认 + 跟进句锚定感情线）
  - 数据：偏好 22 条一条一话题 + 核心记忆 5 条（含《乌色月》原文引用）；split_reply 重做（逗号限制 ≤1、省略号无语保护）
  - 修复：动态 OPUS 提取、推送强制刷好友、自动通过好友申请、梗匹配记入短期记忆
