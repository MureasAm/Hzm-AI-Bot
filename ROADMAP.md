...
- **多句/分段（治本，教训：别用参数压症状）**：4段式根源=提示词"回应之后要回抛问题/补细节/自爆"**明令多句**。治本=重写为"把反问/细节/自嘲**揉进同一句**把话题带下去（'没睡呢，你咋还不睡？'），别另起一句补"。曾试 `SPLIT_MAX_PARTS` 4→**2** 压段数=**治标**，对话变怪（两段被焊成 run-on）→**已还原 4**。保留的确定性修复：`_split_sentences` 省略号加"前文≥4字"条件（"灰泽满……"犹豫前缀不再单独切一条）、纯括号段并入上一段（**单独发"（小声嘀咕）"→对方接不住→模型瞎编"钥匙串"胡话**）、超限分段换行连接（防 run-on）
