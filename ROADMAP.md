...
- **灵活拆分（短碎片合并，治"机械切碎"）**：按标点切分后，**任何 <12 字的分段并入下一段**（最后一段过短并回前段）——防"哦？/睡醒了？/那倒是稀奇……"被切成 4 条戏剧节拍。参考业界：leeguoo"不能每个标点都切一刀"、llm-markdown-whatsapp"merge small chunks + keep trailing questions attached"、openclaw"semantic message aggregation"。实测"哦？睡醒了？那倒是稀奇……"合并成一条自然惊讶。保留的确定性修复：`_split_sentences` 省略号加"前文≥4字"条件（"灰泽满……"犹豫前缀不再单独切一条）、纯括号段并入上一段（**单独发"（小声嘀咕）"→对方接不住→模型瞎编"钥匙串"胡话**）、超限分段换行连接（防 run-on）
- **情绪克制**：提示词"你是在 QQ 跟真人聊天，不是在演舞台剧/回直播弹幕"；少感叹号、少堆"嘛/啦/哦/嗯"、**别每条都以"嗯/哦/啊"开头**（模型默认习惯，样本没教）、结巴消融（`clean_reply` 把"那、那"→"那那"）
...
- 测试 160 个全过。实测（温度 0.85）：话题推进重写后"你睡觉了吗"→"没睡呢，你咋也不睡？"（回答+反问揉成一句）；"嗯"只出现在自然确认（"嗯，晚安"）；身份追问全稳无胡话；温度对比 0.8 有"回放学技术"小胡话+更多省略号，**保持 0.85 不降**
