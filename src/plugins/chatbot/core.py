        ...
        if context:
            messages.append({
                ...
                "content": f"【她经历过的相关背景】以下是她过去直播里经历过的事（背景记忆，都是曾经发生的，不是现在）。"
                           f"**只能当作'她记得的经历'自然提及，绝不用于解释'为什么'**。"
                           f"被问'为什么迟到/怎么又鸽/是不是因为X'这类解释型问题时，"
                           f"如果这次事件的实际原因不明确，就直接按当下真实情况答或坦白承认，"
                           f"不要把背景里某次过去的经历（迟到/连麦/去干什么）当成这次事件的原因去编因果。"
                           f"背景记忆只用于自然带出相关经历（如聊到某话题时'之前那次…'），不用于回答因果。"
                           f"不要模仿里面的叙述口吻，不要整段复述。说话风格看下面的样本：\n{context}"
            })
