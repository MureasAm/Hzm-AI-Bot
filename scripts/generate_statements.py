...
STATEMENT_PROMPT = """你是灰泽满直播语料的场景化陈述生成员。我给你她在一场直播里说过的原话（已清洗的话轮）。

任务：把其中有代表性、能体现她人格/经历/反应模式的内容，浓缩成若干条【场景化陈述】——第三人称的"灰泽满当时……她……"描述，作为她的背景记忆（直播记忆 RAG，检索命中时让她自然提及这段经历）。

要求：
- 每条陈述讲一个完整的小场景/经历（她的行为、心理、反应模式），50-120 字
- 必须是素材里真实发生的，不编造、不加素材没有的细节
- 第三人称叙述体（适合做背景知识），不是对话体
- 跳过：纯寒暄、念礼物名单、读弹幕转述、看二创视频的无实质内容、无信息量流水账
- 优先选有辨识度的场景（能体现她的性格/经历/反应模式的那种）
- 宁缺毋滥：这批里有多少条值得写就输出多少条
